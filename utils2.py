...
def update_progress(rank, current, total, server_name = ''):
    ...
    with open(f'cache/{hostname}progress_{rank}.json', 'w') as f:
        json.dump({"progress": current, "total": total}, f)
def update_sub_progress(rank, current, total,status,upper_level,upper_level_idx):

    # 현재 프로세스의 진행 상황을 파일에 기록
    with open(f'cache/{hostname}subprogress_{rank}.json', 'w') as f:
        json.dump({"progress": current, "total": total,"status" : status,"upper":upper_level,"upper_idx":upper_level_idx}, f)
...
def sub_monitor_progress(world_size, parts):
    ...
    while not all(completed):
        for i in range(world_size):
            progress_file = f'cache/{hostname}progress_{i}.json'
            subprogress_file = f'cache/{hostname}subprogress_{i}.json'
            if os.path.exists(progress_file):
